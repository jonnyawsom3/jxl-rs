...
pub fn adaptive_lf_smoothing(
    ...
    // Chroma has now been expanded onto the native LF pixel grid. Do not keep
    // using the original chroma shifts for the smoothing pass: doing so would
    // only read/write every 2^shift-th sample and leave the newly upsampled
    // pixels untouched, producing blocky and phase-shifted chroma.
    let shifts = shifts.map(|(hshift, vshift)| {
        (if hshift != 0 { 0 } else { hshift }, if vshift != 0 { 0 } else { vshift })
    });
